...
#define DEFAULT_SUSPEND_MAX_FREQ_SILVER 1209600
...
{	
	...
	/*if(policy->cpu < 4 && !is_display_on() &&  silver_max_freq > 0 && silver_max_freq < freq) {
		return silver_max_freq;
	}

	if(policy->cpu > 3 && policy->cpu && !is_display_on() && gold_max_freq > 0 && gold_max_freq < freq) {
...
static int smugov_init(struct cpufreq_policy *policy)
{
    ...
    tunables->hispeed_freq = 2419200;
	}
 
  if (policy->cpu == 7) tunables->hispeed_freq = 2841600;
